...
function install_mysql(){
	...
	if [ "$release" == "8" ];then
		if [ "$mysql_var" = "8" ]; then
			rpm -ivh $DOWNLOAD_FILE_URL/repo/mysql80-community-release-el8-1.noarch.rpm --nodeps --force
...
function XtraBackup()
{
	yum -y install perl perl-devel libev libev-devel rsync perl-Time-HiRes perl-DBD-MySQL perl-Digest-MD5
	if [ "$release" -eq "8" ]; then
		rpm -ivh --force $DOWNLOAD_FILE_URL/file/percona-xtrabackup-24-2.4.23-1.el8.x86_64.rpm
	elif [ "$release" -eq "7" ]; then
		rpm -ivh --force $DOWNLOAD_FILE_URL/file/percona-xtrabackup-24-2.4.23-1.el7.x86_64.rpm
	elif test `arch` = "x86_64"; then
		rpm -ivh --force $DOWNLOAD_FILE_URL/file/percona-xtrabackup-24-2.4.23-1.el6.x86_64.rpm
	else
		rpm -ivh --force $DOWNLOAD_FILE_URL/file/percona-xtrabackup-24-2.4.23-1.el6.i686.rpm
	fi
}
...
————————————————————————————————————————————————————
1. ◎ 自动安装/更新 PHP5.3-8.0（极速安装）
2. ◎ 自动安装/更新 PHP5.3-8.0（编译安装）
3. ◎ 重新安装 PHP5.3-8.0（极速安装）
4. ◎ 重新安装 PHP5.3-8.0（编译安装）
5. ◎ 自动更新 ionCube组件
6. ◎ 自动更新 SG11组件
...
echo -e "————————————————————————————————————————————————————
	\033[1mＫＡＮＧＬＥＳＯＦＴ\033[0m
	\033[32mKangle一键脚本-主菜单-Ver.$KANGLE_VER\033[0m
	说明：kangle命令可随时打开当前菜单
————————————————————————————————————————————————————
